...
function generateTagline(prompt) {
    ...
    if (lowerPrompt.includes('cafe')) return 'Warmth in every cup';
...
async function downloadPreview() {
    ...
    try {
        ...
        // Dodaj logo ako postoji
        if (logoBlob) {
            zip.file("logo.svg", logoBlob);
        }
        
        // Dodaj colors.json
        zip.file("brand-colors.json", JSON.stringify(colors, null, 2));
        
        // Dodaj fonts.json
        zip.file("brand-fonts.json", JSON.stringify({
            heading: fonts.heading,
            body: fonts.body,
            headingStyle: fonts.headingStyle,
            bodyStyle: fonts.bodyStyle
        }, null, 2));
        
        // Dodaj summary.txt
        const summary = `BrandCraft.ai - Brand Identity Summary
================================
...
Fonts:
- Heading: ${fonts.heading} (${fonts.headingStyle})
- Body: ${fonts.body} (${fonts.bodyStyle})

---
Generated by BrandCraft.ai
https://zaklinaradivojevic.github.io/brandcraft
`;
        zip.file("brand-summary.txt", summary);
        
        // Dodaj README
        const readme = `# Your Brand Kit from BrandCraft.ai

## Files included:
- **logo.svg** - Vector logo file
- **brand-colors.json** - Color palette
- **brand-fonts.json** - Font recommendations
- **brand-summary.txt** - Complete brand summary
...
---
Generated by BrandCraft.ai | Your vibe, your brand
`;
        zip.file("README.md", readme);
        
        // Generiši ZIP i download
        const content = await zip.generateAsync({ type: "blob" });
        const link = document.createElement('a');
        const url = URL.createObjectURL(content);
        link.href = url;
        link.download = `brandcraft-${Date.now()}.zip`;
        ...
        alert('✅ Brand kit downloaded! Check your Downloads folder.');
        ...
        const downloadButton = document.getElementById('downloadPreviewBtn');
        downloadButton.textContent = '📥 Download Preview';
        downloadButton.disabled = false;
    }
}
